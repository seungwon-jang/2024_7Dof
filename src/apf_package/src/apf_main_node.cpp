  ...
  private:
    ...
    void topic_callback(const apf_interfaces::msg::Jacobian & msg) 
    {
      ...
      */
      ...
      // topic 시간을 계산한다. 일단 시간을 가져오고,
      auto current_time = this->now();

      if (last_msg_time_.nanoseconds() != 0) {
            // 지난 메시지 시각이 존재하는 경우, 시간 차이를 계산
            auto time_diff = (current_time - last_msg_time_).seconds();
            //RCLCPP_INFO(this->get_logger(), "Time since last message: %f seconds", time_diff);
        }
      // 현재 시각을 마지막 메시지 시각으로 업데이트
